package io.mallang.common.domain.vo;

import io.mallang.common.domain.exception.InvalidValueException;

import java.util.regex.Pattern;

public record Receiver(String name, String phoneNumber) {

    private static final Pattern PHONE_NUMBER_PATTERN = Pattern.compile("010\\d{8}");

    public Receiver {
        if (name == null || name.isBlank()) {
            throw new InvalidValueException("이름을 입력해주세요.");
        }

        if (!name.equals(name.strip())) {
            throw new InvalidValueException("이름은 앞뒤 공백이 없어야 합니다.");
        }

        if (name.length() < 2 || name.length() > 10) {
            throw new InvalidValueException("이름은 2자 이상 10자 이하여야 합니다.");
        }

        if (phoneNumber == null || !PHONE_NUMBER_PATTERN.matcher(phoneNumber).matches()) {
            throw new InvalidValueException("올바른 전화번호 형식이 아닙니다. (010XXXXXXXX)");
        }
    }
}
