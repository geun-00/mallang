...
class CartCommandApiWebMvcTest extends WebMvcRequestTestSupport {
    ...
    class 항목_추가_요청_검증 {
        ...
        @NullSource
        @ValueSource(strings = {"", " "})
        void productId_속성이_지정되지_않으면_400_Bad_Request_상태코드를_반환한다(String invalidProductId) throws JsonProcessingException {
            // given
            var request = new AddCartItemRequest(invalidProductId, 2);

            // when
            MvcTestResult result = postJson(CART_ITEMS_API, request);
        ...
        void quantity가_0_이하이면_400_Bad_Request_상태코드를_반환한다(int invalidQuantity) throws JsonProcessingException {
            // given
    ...
    class 수량_변경_요청_검증 {
        ...
        void quantity_속성이_지정되지_않으면_400_Bad_Request_상태코드를_반환한다() throws JsonProcessingException {
            ...
            // when
            MvcTestResult result = patchJson(CART_ITEMS_API + "/" + "cart-item-id", request);
